...
public class ReceiverApplicationLayer {
  public static void receiverApplicationLayer(int bits[], MainController controller) {
    ...
    for (int i = 0; i < bits.length; i++) {
      controller.addToNumsToAsciiTextField("" + (char) (bits[i]));
      try {
        Thread.sleep(controller.getSpeed());
      } catch (Exception e) {
      }
    ...
    Enlacing enlace = new Enlacing(3);
    String newMsg = enlace.decode(msg, controller);

    controller.addToReceiveMsgTextField(newMsg);
